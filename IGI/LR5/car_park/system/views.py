...
def parking_spaces(request):
	...
	return render(request, 'system/parking_spaces.html', {'parking_spaces': parking_spaces})
...
def client_balance(request):
	...
	if request.method == 'POST':
		...
		for client in clients:
			...
			total = total_accruals - total_payments
...
def get_balance(client: Client):
	...
	try:
		...
		accruals = []
	return _sum(accruals, lambda x: x.amount) - _sum(payments, lambda x: x.total_cost) 
